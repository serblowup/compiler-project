...
./compiler compile --input <файл> --output <файл> [опции]
```
...
5. **IR-генератор** — трансляция AST в промежуточное представление (SSA-форма)
6. **Оптимизатор IR** — свёртка констант, алгебраические упрощения, удаление мёртвого кода
7. **Генератор кода** — трансляция IR в x86-64 ассемблер с распределением регистров (Linear Scan)
